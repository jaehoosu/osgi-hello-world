...
public class Activator implements BundleActivator {
  ...
  public synchronized void start(BundleContext bundleContext) throws Exception {
    Injector injector = Guice.createInjector(Peaberry.osgiModule(bundleContext), new GuiceModule());
    ...
    this.tracker = new ServiceTracker<HttpService, HttpService>(this.bc,
  ...
  private void rawRegisterServlets()
      throws ServletException, NamespaceException, InterruptedException {
    ...
    try {
      ...
      httpService.registerServlet("/services", new ServletContainer(), getJerseyServletParams(),
          null);
    } finally {
      Thread.currentThread().setContextClassLoader(originalContextClassLoader);
    }
    // END of workaround - after grizzly updated to the recent version, only the inner call from try
    // block will remain:
